...
@Component({
  selector: 'RamdorIv',
  templateUrl: './ramdorIv.component.html',
  standalone: false,
  styleUrl: './ramdorIv.component.css'
})
export class RamdorIv implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.searchSubject.pipe(
      ...
      switchMap(filters => this.ramInvService.getInvoices(filters, this.page, this.pageSize))
    ).subscribe(data => {
  ...
  getAllIv() {
    ...
    console.log("Fetching invoices with page:", this.page, "pageSize:", this.pageSize);
    this.subscription = this.ramInvService.getInvoices(this.filters, this.page, this.pageSize).subscribe(data => {
      if (this.page === 1) {
